...
| `scripts/validators/` | Deterministic single-file Node validators, pinned by frozen fixtures. | present in part — `gate-self-test.js` (#21), `cross-reference-integrity.js` (#19), and `link-integrity.js` (#23), plus the check `registry.json`, landed (WP 5.1); the remaining eight WP 5.1 checks forthcoming |
...
| `fixtures/` | Frozen per-validator fixture packs. Retained, not scratch. | present in part — `fixtures/gate-self-test/` (#21), `fixtures/cross-reference-integrity/` (#19), and `fixtures/link-integrity/` (#23) landed (WP 5.1); remaining packs land with their checks; the local-CI parity runner is WP 5.2 (#31) |

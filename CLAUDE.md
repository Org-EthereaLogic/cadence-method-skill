...
| `scripts/validators/` | Deterministic single-file Node validators, pinned by frozen fixtures. | present in part — `gate-self-test.js` (#21) and `cross-reference-integrity.js` (#19), plus the check `registry.json`, landed (WP 5.1); the remaining nine WP 5.1 checks forthcoming |
...
| `fixtures/` | Frozen per-validator fixture packs. Retained, not scratch. | present in part — `fixtures/gate-self-test/` (#21) and `fixtures/cross-reference-integrity/` (#19) landed (WP 5.1); remaining packs land with their checks; the local-CI parity runner is WP 5.2 (#31) |

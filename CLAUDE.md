...
| `scripts/validators/` | Deterministic single-file Node validators, pinned by frozen fixtures. | present in part — `gate-self-test.js` + the check `registry.json` landed via #21 (WP 5.1); the remaining WP 5.1 checks forthcoming |
...
| `fixtures/` | Frozen per-validator fixture packs. Retained, not scratch. | present in part — first pack `fixtures/gate-self-test/` landed via #21 (WP 5.1); remaining packs land with their checks; the local-CI parity runner is WP 5.2 (#31) |

...
| **Status** | v1.11 final — reconciled and ready for design-freeze review alongside the Project Plan & WBS |
...
**As a** practitioner, **I want** a single `/cadence:init` command that seeds a new or existing project with the CADENCE scaffold — Constitution, Directives template, evidence-class reference, ID-namespace reference, manifest template, zone directories, an `artifacts/` evidence root, the gate-tier configuration, and agent/hook/command registrations — **so that** every project inherits the method's shared invariants from the first commit without hand-copying them.

**Acceptance criteria.**

- **AC-1.1** — Given an empty directory, when `/cadence:init` runs, then it creates the scaffold layout (manifest, `cadence/draft/`, `cadence/candidate/`, `cadence/approved/`, `artifacts/` evidence root, `cadence/gate-tiers.json`) and reports each file it created; re-running on the same directory is idempotent — it reports what already exists and overwrites nothing without explicit confirmation (`P6`).
...
- **AC-12.2** — A fixture suite covers each validator's **reachable** verdicts plus its documented edge cases; the suite runs in local CI, and any output difference from the frozen baseline fails the suite. Where a verdict is unreachable by design — loose-pointer drift's `fail`, which method §6.2 rule 1 forbids — that slot holds instead the **falsifiability fixture** pinning the unreachability: an input a naive implementation would get wrong, with the expected verdict recorded as what a correct implementation returns. Demanding the unreachable verdict would mark a correct validator broken.
...
- **AC-14.2** — Consensus is opt-in. Method §6.3 reserves the model review tier for Candidate → Approved; this runtime additionally exposes it at Draft → Candidate as an explicitly invoked, default-off option — a recorded divergence from §6.3's placement rule, never a default. The project configuration records the default per boundary — the `review` block of `cadence/gate-tiers.json`, one `consensus` and one `grader` boolean per boundary, seeded off at both per Q1 — so review depth is a deliberate, visible choice (§6.4 risk-based depth) and the configured default is inspectable via `/cadence:status --gates`. Setting one on is a reviewed configuration edit like any tier change (AC-16.2); it is a default, never a block, and does not touch the rule that model output never changes the deterministic verdict.
...
- **AC-16.1** — A single configuration file, `cadence/gate-tiers.json`, lists every check with its tier and carries the per-boundary review-tier defaults AC-14.2 requires; `/cadence:status --gates` renders both; and a check configured to block outside a promotion boundary is itself a configuration-validation failure.
...
| v1.10 | 2026-08-06 | **AC-12.2 restated: a fixture suite pins each validator's *reachable* verdicts, not a verdict the method forbids it to reach.** The criterion required a suite covering "each validator's `pass`, `warn`, and `fail` paths" for every validator. Loose-pointer drift has no reachable `fail` — method §6.2 rule 1 states that a loose pointer "produces a *warning* when it drifts, never a failure," and method §5 downgrades that cross-reference rule from fail to warn, which is the whole of the cascade fix FR-12 and AC-13.2 carry. A correct implementation of that validator therefore *cannot* produce the fixture the criterion demanded, and the criterion as written would have marked it broken. **What replaces it:** the suite covers each validator's reachable verdicts plus its documented edge cases, and an unreachable slot holds the **falsifiability fixture** that pins the unreachability — an input a naive implementation gets wrong, with the expected verdict recorded as what a correct implementation returns. This adopts, rather than invents, the convention `docs/validator-spec-sheet.md` §7 already specified for exactly this case: loose-pointer drift's fail slot holds a corpus carrying both a drifted live pointer and a stale version inside a revision row, expected `warn` with exactly one finding, which a naive implementation fails two ways — by returning `fail` on the drift, or by reporting a second finding on the historic row. The spec sheet had the right rule and this criterion contradicted it; the criterion is what moves, because the spec sheet is subordinate to this companion. **Nothing is weakened.** Every validator with three reachable verdicts still pins all three; the byte-for-byte baseline diff, the local-CI requirement, and the documented-edge-case obligation are unchanged; and the one validator affected gains a *stricter* fixture than a bare `warn` case would have been, because the falsifiability fixture pins the two distinct ways of getting it wrong. **Found by** an accuracy review of the GitHub issue set against this companion and the spec sheet, recorded with its sibling findings in the authority document's v1.16 row; that document also names NFR-2's reference corpus by path and completes three WBS Depends-on cells. No user story, criterion, or identifier was added, renumbered, or reused; prior revision rows remain unchanged. |
| v1.11 | 2026-08-06 | **The scaffold gains the gate-tier configuration, and the review tier's per-boundary default gains a named home.** Applied jointly with Project Plan & WBS v1.17, which records the full disposition. **US-1 and AC-1.1 gain `cadence/gate-tiers.json`.** The spec sheet and the WBS 5.4 issue both asserted that `/cadence:init` seeds this file, while S-6 — the enumeration AC-1.1's seed set is pinned to under CR-5 — omitted it, so the obligation existed in three places and belonged to no package. S-6 gains it in the same change, so the two documents continue to enumerate the same scaffold and the CR-5 condition does not recur. **AC-14.2 names where the default lives.** The criterion required the project configuration to record the consensus default per boundary and to make it inspectable via `/cadence:status --gates`, without saying which file that is; AC-19.2 and AC-10.3 both lean on it, and WP 6.2 and WP 6.3 both build against it. It is the `review` block of `cadence/gate-tiers.json` — the file `--gates` already renders — carrying one `consensus` and one `grader` boolean per boundary, seeded off at both boundaries per Q1's *opt-in everywhere* resolution. The parenthetical recommendation (on, for authority documents at Candidate → Approved) is retained as a recommendation and is deliberately **not** made the seeded value: it is scoped to authority documents, the schema carries no document-class dimension, and inventing one to hold a recommendation would widen it to every document at that boundary. **AC-16.1 admits the block and names the file**, so a validator built to reject unknown content does not reject a legitimate `review` block, and so the criterion no longer describes a file that lists only checks. **Nothing here weakens the advisory rule.** A recorded default is a default: X-3, FR-13, AC-14.1, and AC-19.2 are untouched, and model output still never changes the deterministic verdict or blocks promotion by itself. No user story or criterion was added, renumbered, or reused; no identifier was introduced; prior revision rows remain unchanged. |

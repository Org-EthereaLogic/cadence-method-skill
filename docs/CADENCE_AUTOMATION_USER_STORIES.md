...
| **Status** | v1.7 final — reconciled and ready for design-freeze review alongside the Project Plan & WBS |
...
- **AC-11.1** — Given any write to a Draft-zone file, the pre-write hook always allows the write; its advisories appear as feedback; and the advisory pass completes in under 1 second at p95, measured by the WBS 7.3 drill using NFR-2's reference-host record, warm-up count, sample size, and percentile method (§6.1: "sub-second, host-only").

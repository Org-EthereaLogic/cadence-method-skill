...
| --- | --- | --- | --- | --- | --- |
| `init` | `/cadence:init` | `skills/init/SKILL.md`; legacy alt. `commands/init.md` | `$cadence-init` (`@cadence-init` in ChatGPT); implicit selection; `/skills` listing | `.agents/skills/cadence-init/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `frame` | `/cadence:frame` | `skills/frame/SKILL.md`; legacy alt. `commands/frame.md` | `$cadence-frame`; implicit selection; `/skills` listing | `.agents/skills/cadence-frame/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `assess` | `/cadence:assess` | `skills/assess/SKILL.md`; legacy alt. `commands/assess.md` | `$cadence-assess`; implicit selection; `/skills` listing | `.agents/skills/cadence-assess/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `innovate` | `/cadence:innovate` | `skills/innovate/SKILL.md`; legacy alt. `commands/innovate.md` | `$cadence-innovate`; implicit selection; `/skills` listing | `.agents/skills/cadence-innovate/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `model` | `/cadence:model` | `skills/model/SKILL.md`; legacy alt. `commands/model.md` | `$cadence-model`; implicit selection; `/skills` listing | `.agents/skills/cadence-model/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `implement` | `/cadence:implement` | `skills/implement/SKILL.md`; legacy alt. `commands/implement.md` | `$cadence-implement`; implicit selection; `/skills` listing | `.agents/skills/cadence-implement/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `track` | `/cadence:track` | `skills/track/SKILL.md`; legacy alt. `commands/track.md` | `$cadence-track`; implicit selection; `/skills` listing | `.agents/skills/cadence-track/SKILL.md` | Packaged-file choice: §9(a) — decided (Q5). |
| `gate` | `/cadence:gate` | `skills/gate/SKILL.md`; legacy alt. `commands/gate.md` | `$cadence-gate`; implicit selection; `/skills` listing | `.agents/skills/cadence-gate/SKILL.md` | Standalone advisory run (FR-14, WBS 6.1); packaged-file choice: §9(a) — decided (Q5). |
| `promote` | `/cadence:promote` | `skills/promote/SKILL.md`; legacy alt. `commands/promote.md` | `$cadence-promote`; implicit selection; `/skills` listing | `.agents/skills/cadence-promote/SKILL.md` | `--finalize` is a flag on this action (FR-6), not an eleventh action; packaged-file choice: §9(a) — decided (Q5). |
| `status` | `/cadence:status` | `skills/status/SKILL.md`; legacy alt. `commands/status.md` | `$cadence-status`; implicit selection; `/skills` listing | `.agents/skills/cadence-status/SKILL.md` | Read-only report (FR-2); packaged-file choice: §9(a) — decided (Q5). |
...
> **Resolved 2026-08-06 UTC — registered as Q5 and decided against its pending default: the ten actions ship as `skills/<action>/SKILL.md`.** The live documentation directs new plugins to `skills/`, `commands/` is the older of the two supported surfaces, and no action was built yet, so the migration cost was zero at the moment of choosing and rises with each action authored. §5's packaged-file column now names the decided surface first and `commands/*.md` as the legacy alternative. The invocation string is unchanged. One correction this resolution carries: the bearing statement above, and Q5's first wording, named WP 3.1, 3.2 and 4.1–4.6 — eight packages. The ten actions include `gate` and `promote`, so **WP 6.1 and WP 6.3 are also in scope**; S-2 has always counted them among the ten. See §7 of the authority document and its v1.13 Revision Record row.
...
> **Resolved 2026-08-06 UTC — registered as Q6 and decided by adopting its pending default: `PostToolUse` with a `Write|Edit` matcher.** The advisory-only guarantee D-2 and FR-7 state now holds by construction — a non-blockable event cannot block — at the stated cost that annotation follows the write rather than preceding it. The third option was not taken: the Codex side is an unverified open item (§8), and deferring a Claude Code decision on it would leave WP 5.3 unbuildable while WBS 5.0 is ungated. **Beyond Q6's own wording:** S-4 places the Approved-zone write warning in the same work package under the same all-hooks-warn-only rule, so both WP 5.3 hooks bind non-blockably; that follows the decision rather than extending it, and the authority document records it for the freeze to confirm. See §7 of the authority document and its v1.14 Revision Record row, and `CADENCE_AUTOMATION_USER_STORIES.md` v1.9 for the restated AC-11.1.

Neither divergence was registered as a new `Q` identifier when this map shipped: doing so would have required editing §7 of the authority document, a second edit beyond the one Revision Record row this change's criterion 8 scoped it to (FR-10 also requires an identifier's definition to land in the same change that introduces it).

**Subsequently registered.** Divergence (a) is now **Q5** and divergence (b) is now **Q6** in §7 of the authority document, and WP 1.4's resolve cell reads `Q1/Q2/Q4/Q5/Q6`; see that document's v1.12 Revision Record row. Each carries a *default pending decision* — the fallback an implementer follows until the freeze decides — which is not the decision. This section is unchanged on the substance and still decides neither; the authority document holds the canonical definitions (FR-10).
...
- ~~**`CONTRIBUTING.md` still calls the `commands/`-versus-`skills/` choice a WP 1.5 determination**, while this map records it for the WP 1.4 freeze instead (§9(a)). `CONTRIBUTING.md` lies outside this change's write scope (`policy.allowed_paths`), so the drift is recorded here rather than corrected there.~~ **Closed 2026-08-06 UTC:** `CONTRIBUTING.md` now records the resolved Q5 decision. Struck rather than deleted, because a stated limit that quietly disappears is worth less than one that shows when and how it closed.

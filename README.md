...
**Release:** pre-release. **M1 taken** — the design freeze was signed 2026-08-06 UTC ([docs/design-freeze.md](docs/design-freeze.md)) and WBS 1.0 is complete. WBS 2.0 (Skill Core) is complete; WBS 3.0 is underway (WP 3.1 landed the first practitioner command) and WBS 5.0 is landing in parallel — six of the eleven WP 5.1 validators are built: `gate-self-test` (#21), `cross-reference-integrity` (#19), `link-integrity` (#23), `id-namespace-resolution` (#22), `evidence-tag-grammar` (#20), and `loose-pointer-drift` (#24). WBS 3.0–7.0 continue toward M3 (release 1). No installable release is published, and the promotion gate does not exist yet, so the method's blocking controls are not yet enforceable — a landed validator is a script no gate tier invokes.
...
One of the ten commands is built, so a local load gives you `/cadence:init` and nothing else. The promotion gate and the phase commands do not exist yet. Six of the eleven WP 5.1 validators have landed as scripts under `scripts/validators/`, but no command surface invokes them and no gate tier is configured, so they block nothing — see Status above for what has landed.

...
const MUTATIONS = [
  ...
  },
  {
    ...
    find: '    if (rows === 1 && arts === 1) {',
    repl: '    if (true) {'
  },
  {
    id: 'ambiguous basename join resolved instead of refused',
    rule: 'a basename shared by two governed artifacts is refused, not resolved',
    find: '    if (rows === 1 && arts === 1) {',
    repl: '    if (rows === 1 && arts >= 1) {'
  },
  {
  ...
  },
  {
    ...
    rule: 'a row whose exact path moved keeps existing via its basename (a zone move)',
    find: '    if (!exists && artifactBasenames.has(e.basename)) {',
    repl: '    if (false) {'
  },
  {
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mrc-mutation-'));
try {
